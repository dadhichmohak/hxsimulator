#!/usr/bin/env python3
"""Build app.py by writing it in manageable chunks."""
import os
...
from hx_simulator.heat_transfer import FOULING_FACTORS as FF
...
.hx-ok    { background: rgba(0,200,83,0.15);    color: #00c853; border: 1px solid rgba(0,200,83,0.3); }
...
const stTheme = document.querySelector('[data-theme]');
const currentTheme = stTheme ? stTheme.getAttribute('data-theme') : (isDark ? 'dark' : 'light');
...
def get_tc():
    if is_dark():
        ...
        return dict(bg="none", text="#333", grid="#ccc", hot="#d32f2f",
                    cold="#1976d2", accent="#e65100", fill="#ffcc80",
                    line2="#2e7d32", dot="#000000")
...
# ── Part 3: SVG visualization function ──────────────────────────────────
parts.append(r'''
def render_hx_svg(hx_type, arr_key, Th_in, Th_out, Tc_in, Tc_out,
    ...
    s.append(f'<rect x="{TL-5}" y="{ST-5}" width="{TR-TL+10}" height="{SB-ST+10}" '
             f'rx="14" fill="none" stroke="#888" stroke-width="2.5" opacity="0.7"/>')
    ...
    s.append(f'<line x1="{TL+30}" y1="{CY}" x2="{TR-30}" y2="{CY}" '
             f'stroke="white" stroke-width="1.5" stroke-dasharray="6,8" opacity="0.4" '
    ...
    if baffle and hx_type == "Shell-and-tube":
        ...
        for i in range(1, n_b + 1):
            ...
            s.append(f'<line x1="{bx}" y1="{btop}" x2="{bx}" y2="{bbot}" '
                     f'stroke="#666" stroke-width="2" opacity="0.5" stroke-dasharray="4,3"/>')
    ...
    s.append(f'<text x="{TL-8}" y="{CY+10}" text-anchor="end" font-size="9" fill="#999">Hot In</text>')
    # Hot out (right)
    s.append(f'<text x="{TR+8}" y="{CY-3}" text-anchor="start" font-size="{fs}" '
             f'font-weight="600" fill="{h_oc}">{Th_out-273.15:.1f}</text>')
    s.append(f'<text x="{TR+8}" y="{CY+10}" text-anchor="start" font-size="9" fill="#999">Hot Out</text>')
    ...
    if arr_key == "counter":
        ...
        s.append(f'<text x="{TR+8}" y="{CY+42}" text-anchor="start" font-size="9" fill="#999">Cold In</text>')
        s.append(f'<text x="{TL-8}" y="{CY+29}" text-anchor="end" font-size="{fs}" '
                 f'font-weight="600" fill="{c_oc}">{Tc_out-273.15:.1f}</text>')
        s.append(f'<text x="{TL-8}" y="{CY+42}" text-anchor="end" font-size="9" fill="#999">Cold Out</text>')
    else:
        s.append(f'<text x="{TL-8}" y="{CY+29}" text-anchor="end" font-size="{fs}" '
                 f'font-weight="600" fill="{c_ic}">{Tc_in-273.15:.1f}</text>')
        s.append(f'<text x="{TL-8}" y="{CY+42}" text-anchor="end" font-size="9" fill="#999">Cold In</text>')
        s.append(f'<text x="{TR+8}" y="{CY+29}" text-anchor="start" font-size="{fs}" '
                 f'font-weight="600" fill="{c_oc}">{Tc_out-273.15:.1f}</text>')
        s.append(f'<text x="{TR+8}" y="{CY+42}" text-anchor="start" font-size="9" fill="#999">Cold Out</text>')

    # Title
    s.append(f'<text x="{(TL+TR)//2}" y="22" text-anchor="middle" font-size="13" '
             f'font-weight="700" fill="#ccc">{hx_type} ({arr_key}-flow)</text>')

    # Parameter box (bottom right)
    bx, by = TL + 10, SB + 20
    s.append(f'<rect x="{bx}" y="{by}" width="195" height="82" rx="6" '
             f'fill="rgba(0,0,0,0.35)" stroke="rgba(255,255,255,0.1)" stroke-width="1"/>')
    info_lines = [
        f"q = {q:,.0f} W",
        f"h_i = {h_i:.0f} W/m\u00b2K",
        f"h_o = {h_o:.0f} W/m\u00b2K",
    ...
    for i, line in enumerate(info_lines):
        s.append(f'<text x="{bx+8}" y="{by+15+i*14}" font-size="10" fill="#ddd">{line}</text>')
    ...
    if baffle:
        ...
        for i, line in enumerate(g_lines):
            s.append(f'<text x="{gx+8}" y="{by+15+i*14}" font-size="10" fill="#ddd">{line}</text>')
    else:
        gx = (TL+TR)//2 - 90
...
parts.append(r'''
    ...
    with tab_foul:
        ...
        if result.fouling_pred:
            ...
            with ff1:
                st.metric("Design Rf", f"{fp.Rf_design:.6f} m2K/W")
                st.metric("Predicted Rf (5yr)", f"{fp.Rf_predicted:.6f} m2K/W")
            with ff2:
                st.metric("U clean", f"{fp.U_clean:.0f} W/m2K")
                st.metric("U fouled", f"{fp.U_fouled:.0f} W/m2K")
            with ff3:
                st.metric("U minimum", f"{fp.U_minimum:.0f} W/m2K")
                margin = fp.U_fouled - fp.U_minimum
                st.metric("Fouling margin", f"{margin:.0f} W/m2K")
            with ff4:
                st.metric("Cleaning interval", f"{fp.cleaning_interval:.1f} years")
                st.metric("Remaining life", f"{fp.lifetime_years:.0f} years")
        else:
            st.info("Fouling prediction is available for shell-and-tube exchangers only.")
...
print(f"Wrote {len(full)} bytes to {APP_PATH}")
